...
const PurposeTree = ({ settingsView = false }) => {
  ...
  const handleDialogConfirm = () => {
    ...
    const selectedTransformations = Object.keys(transformations).filter(
        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            <Checkbox
              ...
              disabled={disabled}
            />
            <div style={{ color: disabled ? "grey" : "inherit" }}>
              {node.name}
            </div>
          </div>
          {settingsView && (
            <div>
              <IconButton
                color="primary"
                onClick={() => handleAddNode(node.purpose_id)}
              >
                <AddCircleOutlineIcon />
              </IconButton>
              <IconButton
                color="secondary"
                onClick={() => handleDeleteNodeContext(node.purpose_id)}
              >
  ...
  const renderTree = (nodes, parentSelected = true) =>
    nodes.map((node) => (
      <TreeItem
        ...
        label={renderLabel({
          ...node,
          parentSelected: node.selected && parentSelected,
        })}
      >
        {Array.isArray(node.children)
          ? renderTree(node.children, node.selected && parentSelected)
          : null}
      </TreeItem>
    ));
